...
with st.sidebar:
    ...
    st.metric("현재 나이", f"{cur_age:.1f} 세")
    
    # 부모 정보
    st.markdown("---")
    show_parent_info = st.checkbox("부모 정보 입력")
...
# 탭으로 주요 기능 분리
tab1, tab2, tab3, tab4 = st.tabs(["📝 데이터 입력", "📊 시각화", "🔮 예측 분석", "⚙️ 설정"])
...
with tab1:
    ...
    # 🔹 입력 선택
    data_type = st.radio("**입력 선택**", ["안축장", "굴절이상", "각막곡률", "각막두께", "없음"], horizontal=True)

if data_type == "없음":
    st.info("데이터 입력을 선택하지 않았습니다. 위에서 원하는 데이터 타입을 선택해주세요.")
    
elif data_type == "안축장":
    # 입력 방식 선택
    axl_input_method = st.radio("**입력 방식**", ["선택입력", "텍스트입력", "이미지(OCR)"], horizontal=True)
    
    if axl_input_method == "선택입력":
        st.markdown("##### 안축장 선택입력")
        
        # 기본 안축장 입력
        col1, col2, col3 = st.columns(3)
        with col1:
            axl_date = st.date_input("검사일")
        with col2:
        ...
        axl_remarks = st.multiselect("치료/관리", REMARK_OPTIONS, default=[])
        ...
        if st.button("추가", use_container_width=True):
            ...
            st.success("안축장 데이터 추가됨")
            if name: save_bundle(name)
            st.rerun()
    
    elif axl_input_method == "텍스트입력":
        st.markdown("##### 안축장 텍스트입력")
        st.caption("형식: YYYY-M-D, OD(mm), OS(mm)[, Remarks]")
        st.caption("예시: 2025-8-16, 23.25, 23.27, AT; DIMS")
        ...
        with col2:
            if st.button("모두 지우기", type="secondary", use_container_width=True):
                ...
                st.info("안축장 데이터를 모두 비웠습니다.")
                if name: save_bundle(name)
    
    else:  # 이미지(OCR)
        st.markdown("##### 안축장도 이미지 OCR 추출")
        st.caption("안축장도 측정 결과 이미지를 업로드하면 OD, OS의 AL 값을 자동으로 추출합니다.")
        axl_img = st.file_uploader("안축장도 이미지", type=["png","jpg","jpeg"], key="axl_img")
        ...
        if axl_img is not None:
            try:
                ...
                if success:
                    st.success("안축장 데이터 추출 완료!")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if od_al is not None:
                            st.write(f"**우안(OD) AL**: {od_al:.2f} mm")
                        else:
                            st.write("**우안(OD)**: 추출 실패")
                    with col2:
                        if os_al is not None:
                            st.write(f"**좌안(OS) AL**: {os_al:.2f} mm")
                        else:
                            st.write("**좌안(OS)**: 추출 실패")
                    
                    # 날짜 및 추가 설정
                    col1, col2 = st.columns(2)
                    with col1:
                        axl_ocr_date = st.date_input("검사일", value=date.today(), key="axl_ocr_date")
                    with col2:
                        axl_ocr_remarks = st.multiselect("치료/관리 (OCR)", REMARK_OPTIONS, default=[], key="axl_ocr_remarks")
                    
                    # 수동 수정 옵션
                    st.markdown("**수동 수정 (필요시)**")
...
st.markdown(
    """
    **메모**  
    - 안축장은 항상 굴절이상과 일치하는 것은 아닙니다.
    - 각막곡률(K1, K2, Mean K)은 우안(OD)과 좌안(OS)으로 구분하여 저장됩니다.
    - 각막두께는 마이크로미터(μm) 단위로 우안(OD)과 좌안(OS)으로 구분하여 저장됩니다.
    - 자세한 내용은 김응수 교수님께 직접 문의하시기 바랍니다. 
    - 안축장 `data.csv`, 굴절이상 `re_data.csv`, 각막곡률 `k_data.csv`, 각막두께 `ct_data.csv`로 별도 저장됩니다.  
    - 굴절이상은 **구면대응(SE = S + C/2)**를 추세/예측에 사용합니다. 축(Axis)은 저장되지만 예측에는 사용하지 않습니다.  
    """,
    unsafe_allow_html=True
)
